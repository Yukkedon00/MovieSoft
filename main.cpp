...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	w.lpszClassName = _T("MovieTransrate");//アプリケーションクラス名(適当でいいです)
	w.hInstance = GetModuleHandle(0);//ハンドルの取得
	RegisterClassEx(&w);//アプリケーションクラス(こういうの作るからよろしくってOSに予告する)

	RECT wrc = { 0,0, window_width, window_height };//ウィンドウサイズを決める
	AdjustWindowRect(&wrc, WS_POPUP, false);//ウィンドウのサイズはちょっと面倒なので関数を使って補正する
	//ウィンドウオブジェクトの生成
	HWND hwnd = CreateWindowEx(
		WS_EX_LAYERED,//拡張スタイル
		w.lpszClassName,//クラス名指定
		_T("MovieTransrate"),//タイトルバーの文字
		WS_POPUP,//タイトルバーと境界線がないウィンドウです
	...
	_dxgiFactory->CreateSwapChainForComposition(
		_cmdQueue,		// Swap chain needs the queue so that it can force a flush on it.
		&swapchainDesc,
		nullptr,
		(IDXGISwapChain1**)&_swapchain
	);
...
// Create the DirectComposition device
	...
	hr = m_dcompVisual->SetContent(_swapchain);
	if (FAILED(hr)) {
		return -1;
	}
	...
	resdesc.MipLevels = 1;
	resdesc.Format = DXGI_FORMAT_UNKNOWN;				// テクスチャデータはないので不明
	...
	//
	gpipeline.BlendState = AlphaBlend;
	...
	rootparam.ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;//ピクセルシ ェーダから見える
	...
	result = LoadFromWICFile(L"img/test.png", WIC_FLAGS_NONE, &metadata, scratchImg);
	...
	result = _dev->CreateCommittedResource(
		...
		&resDesc,
		D3D12_RESOURCE_STATE_GENERIC_READ,//テクスチャ用(ピクセルシェーダから見る用)
		nullptr,
